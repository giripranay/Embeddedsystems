...
x.trim();
  ...
  {
    int flag=0,m=WiFi.RSSI(0),newnode;
    Serial.println("Available NODES");
    for (int i = 0; i < n &&WiFi.SSID(i)[3]=='.'&& WiFi.SSID(i)[10]!=request[1] && WiFi.SSID(i)[10]!=request[2]; ++i)
    {
      Serial.print(i + 1);
      Serial.print(": ");
      Serial.print("NODE ");
      Serial.print(WiFi.SSID(i)[10]);
      Serial.print(" (");
   ...
   /*   if(WiFi.SSID(i)==x)
      ...
      Serial.print(WiFi.SSID(newnode)[10]);
      Serial.print(" (");
      Serial.print(WiFi.RSSI(newnode));
      Serial.println(")");
   ...
   /*  if(flag==0)
    ...
    x=WiFi.SSID(newnode);
    n_id=int(WiFi.SSID(newnode)[10])-48;
    }
  x.trim();
